const ticketService = require("../services/ticketService")

class TicketController {

<<<<<<< HEAD
  getAll(req, res) {
    try {
      const tickets = ticketService.getAllTickets()
=======
  async getAll(req, res) {
    try {
      const tickets = await ticketService.getAllTickets()
>>>>>>> origin/feature/database
      return res.status(200).json(tickets)
    } catch (error) {
      return res.status(500).json({ message: error.message })
    }
  }

<<<<<<< HEAD
  getById(req, res) {
    try {
      const ticket = ticketService.getTicketById(Number(req.params.id))
=======
  async getById(req, res) {
    try {
      const ticket = await ticketService.getTicketById(Number(req.params.id))
>>>>>>> origin/feature/database
      return res.status(200).json(ticket)
    } catch (error) {
      return res.status(404).json({ message: error.message })
    }
  }

<<<<<<< HEAD
  create(req, res) {
    try {
      const ticket = ticketService.createTicket(req.body)
=======
  async create(req, res) {
    try {
      const ticket = await ticketService.createTicket(req.body)
>>>>>>> origin/feature/database
      return res.status(201).json(ticket)
    } catch (error) {
      return res.status(400).json({ message: error.message })
    }
  }

<<<<<<< HEAD
  updateStatus(req, res) {
    try {
      const ticket = ticketService.updateStatus(
        Number(req.params.id),
        req.body.status
      )

      return res.status(200).json(ticket)
    } catch (error) {
      const status = error.message.includes("não encontrado") ? 404 : 400
      return res.status(status).json({ message: error.message })
    }
  }

  assignBibliotecario(req, res) {
    try {
      const ticket = ticketService.assignBibliotecario(
        Number(req.params.id),
        req.body.bibliotecario
      )

      return res.status(200).json(ticket)
    } catch (error) {
      const status = error.message.includes("não encontrado") ? 404 : 400
      return res.status(status).json({ message: error.message })
    }
  }

  addFeedback(req, res) {
    try {
      const feedback = ticketService.addFeedback(
        Number(req.params.id),
        req.body
      )

      return res.status(201).json(feedback)
    } catch (error) {
      const status = error.message.includes("não encontrado") ? 404 : 400
      return res.status(status).json({ message: error.message })
    }
  }

  uploadTcc(req, res) {
    try {
      const arquivo = ticketService.uploadTccFile(
        Number(req.params.id),
        req.file
      )

      return res.status(201).json({
        message: "Arquivo do TCC enviado com sucesso",
        arquivo
      })
    } catch (error) {
      const status = error.message.includes("não encontrado") ? 404 : 400
      return res.status(status).json({ message: error.message })
    }
  }

  uploadFeedbackFile(req, res) {
    try {
      const arquivo = ticketService.uploadFeedbackFile(
        Number(req.params.id),
        req.file,
        req.body
      )

      return res.status(201).json({
        message: "Arquivo de feedback enviado com sucesso",
        arquivo
      })
    } catch (error) {
      const status = error.message.includes("não encontrado") ? 404 : 400
      return res.status(status).json({ message: error.message })
    }
  }

  delete(req, res) {
    try {
      ticketService.deleteTicket(Number(req.params.id))
      return res.status(204).send()
    } catch (error) {
=======
  async updateStatus(req, res) {
    try {
      const ticket = await ticketService.updateStatus(
        Number(req.params.id),
        req.body.status
      )
      return res.status(200).json(ticket)
    } catch (error) {
      const status = error.message.includes("não encontrado") ? 404 : 400
      return res.status(status).json({ message: error.message })
    }
  }

  async assignBibliotecario(req, res) {
    try {
      const ticket = await ticketService.assignBibliotecario(
        Number(req.params.id),
        req.body.bibliotecario
      )
      return res.status(200).json(ticket)
    } catch (error) {
      const status = error.message.includes("não encontrado") ? 404 : 400
      return res.status(status).json({ message: error.message })
    }
  }

  async addFeedback(req, res) {
    try {
      const feedback = await ticketService.addFeedback(
        Number(req.params.id),
        req.body
      )
      return res.status(201).json(feedback)
    } catch (error) {
      const status = error.message.includes("não encontrado") ? 404 : 400
      return res.status(status).json({ message: error.message })
    }
  }

  async delete(req, res) {
    try {
      await ticketService.deleteTicket(Number(req.params.id))
      return res.status(204).send()
    } catch (error) {
>>>>>>> origin/feature/database
      return res.status(404).json({ message: error.message })
    }
  }
}

module.exports = new TicketController()